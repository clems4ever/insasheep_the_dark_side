...
static INLINE void
lm32bf_fill_argbuf (const SIM_CPU *cpu, ARGBUF *abuf, const IDESC *idesc,
...
static INLINE void
lm32bf_fill_argbuf_tp (const SIM_CPU *cpu, ARGBUF *abuf,
...
static INLINE void
lm32bf_emit_before (SIM_CPU *current_cpu, SCACHE *sc, PCADDR pc, int first_p)
{
  ARGBUF *abuf = &sc[0].argbuf;
  const IDESC *id = & CPU_IDESC (current_cpu) [LM32BF_INSN_X_BEFORE];

  abuf->fields.before.first_p = first_p;
  lm32bf_fill_argbuf (current_cpu, abuf, id, pc, 0);
...
static INLINE void
lm32bf_emit_after (SIM_CPU *current_cpu, SCACHE *sc, PCADDR pc)
{
  ARGBUF *abuf = &sc[0].argbuf;
  const IDESC *id = & CPU_IDESC (current_cpu) [LM32BF_INSN_X_AFTER];
...
{
  const IDESC *id = lm32bf_decode (current_cpu, pc, insn, insn, abuf);

  lm32bf_fill_argbuf (current_cpu, abuf, id, pc, fast_p);
  if (! fast_p)
    {
      int trace_p = PC_IN_TRACE_RANGE_P (current_cpu, pc);
      int profile_p = PC_IN_PROFILE_RANGE_P (current_cpu, pc);
      lm32bf_fill_argbuf_tp (current_cpu, abuf, trace_p, profile_p);
    }
  return id;
}
  ...
  if (fast_p)
      ...
      if (! virtual_p)
	{
	  ...
	  if (PROFILE_MODEL_P (current_cpu)
	    ...
	    lm32bf_model_insn_before (current_cpu, 1 /*first_p*/);
      ...
      if (! virtual_p)
	{
	  ...
	  if (PROFILE_MODEL_P (current_cpu)
	      && ARGBUF_PROFILE_P (abuf))
	    {
	      ...
	      lm32bf_model_insn_after (current_cpu, 1 /*last_p*/, cycles);
	    }
...
INLINE SEM_PC
lm32bf_pbb_begin (SIM_CPU *current_cpu, int FAST_P)
{
  SEM_PC new_vpc;
  PCADDR pc;
  SCACHE *sc;
...
/* end extract-pbb */
	...
	if (_cti_sc)
	  {
	    id = & CPU_IDESC (current_cpu) [LM32BF_INSN_X_CTI_CHAIN];
	  }
	else
	  {
	    id = & CPU_IDESC (current_cpu) [LM32BF_INSN_X_CHAIN];
	  }
...
INLINE SEM_PC
lm32bf_pbb_chain (SIM_CPU *current_cpu, SEM_ARG sem_arg)
{
...
INLINE SEM_PC
lm32bf_pbb_cti_chain (SIM_CPU *current_cpu, SEM_ARG sem_arg,
...
void
lm32bf_pbb_before (SIM_CPU *current_cpu, SCACHE *sc)
{
  SEM_ARG sem_arg = sc;
	  ...
	  if (ARGBUF_PROFILE_P (prev_abuf))
	    {
	      ...
	      lm32bf_model_insn_after (current_cpu, 0 /*last_p*/, cycles);
	    }
	}
  ...
  if (PROFILE_MODEL_P (current_cpu)
    ...
    lm32bf_model_insn_before (current_cpu, first_p);
...
void
lm32bf_pbb_after (SIM_CPU *current_cpu, SCACHE *sc)
{
  SEM_ARG sem_arg = sc;
  const ARGBUF *abuf = SEM_ARGBUF (sem_arg);
  const SEM_ARG prev_sem_arg = sc - 1;
...
void
lm32bf_engine_run_full (SIM_CPU *current_cpu)
{
...
#endif
#else
      lm32bf_sem_init_idesc_table (current_cpu);
#endif

      /* Initialize the "begin (compile) a pbb" virtual insn.  */
      vpc = CPU_SCACHE_PBB_BEGIN (current_cpu);
      SEM_SET_FULL_CODE (SEM_ARGBUF (vpc),
			 & CPU_IDESC (current_cpu) [LM32BF_INSN_X_BEGIN]);
      vpc->argbuf.idesc = & CPU_IDESC (current_cpu) [LM32BF_INSN_X_BEGIN];
...
void
lm32bf_engine_run_fast (SIM_CPU *current_cpu)
{
  SIM_DESC current_state = CPU_STATE (current_cpu);
  SCACHE *scache = CPU_SCACHE_CACHE (current_cpu);
  /* virtual program counter */
...
#endif
#else
      lm32bf_semf_init_idesc_table (current_cpu);
#endif
