...
/* 32 bit ELF emulation code for elf32lm32
...
static void gldelf32lm32_before_parse (void);
static void gldelf32lm32_after_open (void);
static void gldelf32lm32_before_allocation (void);
static void gldelf32lm32_after_allocation (void);
static lang_output_section_statement_type *gldelf32lm32_place_orphan
  (asection *, const char *, int);

static void
gldelf32lm32_map_segments (bfd_boolean need_layout)
{
  int tries = 10;
...
static void
gldelf32lm32_before_parse (void)
{
...
static bfd_boolean
gldelf32lm32_load_symbols (lang_input_statement_type *entry)
{
  int link_class = 0;
...
static void
gldelf32lm32_vercheck (lang_input_statement_type *s)
{
  const char *soname;
...
static void
gldelf32lm32_stat_needed (lang_input_statement_type *s)
{
  struct stat st;
  const char *suffix;
  const char *soname;
...
static bfd_boolean
gldelf32lm32_try_needed (struct dt_needed *needed,
				 int force)
{
  bfd *abfd;
  ...
  if (! force)
      ...
      if (needs != NULL)
	{
	  ...
	  lang_for_each_input_file (gldelf32lm32_vercheck);
	  if (global_vercheck_failed)
	    {
	      bfd_close (abfd);
  ...
  global_found = NULL;
  lang_for_each_input_file (gldelf32lm32_stat_needed);
  if (global_found != NULL)
    {
...
static bfd_boolean
gldelf32lm32_search_needed (const char *path,
				    struct dt_needed *n, int force)
{
  ...
  if (name[0] == '/')
    return gldelf32lm32_try_needed (n, force);
  ...
  while (1)
      ...
      else
	{
      ...
      if (gldelf32lm32_try_needed (&needed, force))
	return TRUE;
...
static void
gldelf32lm32_check_needed (lang_input_statement_type *s)
{
  const char *soname;
...
gldelf32lm32_id_note_section_size (bfd *abfd,
...
static bfd_boolean
gldelf32lm32_write_build_id_section (bfd *abfd)
{
...
gldelf32lm32_after_open (void)
{
  ...
  if (link_info.emit_note_gnu_build_id)
	...
	{
	  size = gldelf32lm32_id_note_section_size (abfd, &link_info);
	  if (size == 0)
	    {
	      ...
	      if (s != NULL && bfd_set_section_alignment (abfd, s, 2))
		{
		  ...
		  t->after_write_object_contents
		    = &gldelf32lm32_write_build_id_section;
  ...
  for (l = needed; l != NULL; l = l->next)
	...
	continue;
      ...
      global_needed = l;
      global_found = NULL;
      lang_for_each_input_file (gldelf32lm32_check_needed);
      if (global_found != NULL
	  && (bfd_elf_get_dyn_lib_class (global_found->the_bfd)
	      & DYN_AS_NEEDED) == 0)
	continue;
      ...
      if (global_found != NULL)
	{
	  ...
	  if (gldelf32lm32_try_needed (&nn, TRUE))
	    continue;
	}
	 ...
	 their use.  See gldelf32lm32_vercheck comment.  */
      for (force = 0; force < 2; force++)
	{
	  size_t len;
	  search_dirs_type *search;

	  if (gldelf32lm32_search_needed (command_line.rpath_link,
						  &n, force))
	    break;
	  len = strlen (l->name);
	  for (search = search_head; search != NULL; search = search->next)
		...
		continue;
	      ...
	      nn.name = filename;
	      if (gldelf32lm32_try_needed (&nn, force))
		break;
	      free (filename);
	    }
	  if (search != NULL)
...
static void
gldelf32lm32_find_exp_assignment (etree_type *exp)
{
  ...
  switch (exp->type.node_class)
      ...
      if (strcmp (exp->assign.dst, ".") != 0)
	{
      ...
      gldelf32lm32_find_exp_assignment (exp->assign.src);
      break;

    case etree_binary:
      gldelf32lm32_find_exp_assignment (exp->binary.lhs);
      gldelf32lm32_find_exp_assignment (exp->binary.rhs);
      break;

    case etree_trinary:
      gldelf32lm32_find_exp_assignment (exp->trinary.cond);
      gldelf32lm32_find_exp_assignment (exp->trinary.lhs);
      gldelf32lm32_find_exp_assignment (exp->trinary.rhs);
      break;

    case etree_unary:
      gldelf32lm32_find_exp_assignment (exp->unary.child);
      break;
...
static void
gldelf32lm32_find_statement_assignment (lang_statement_union_type *s)
{
  if (s->header.type == lang_assignment_statement_enum)
    gldelf32lm32_find_exp_assignment (s->assignment_statement.exp);
}


/* used by before_allocation and handle_option. */
static void 
gldelf32lm32_append_to_separated_string (char **to, char *op_arg)
{
  if (*to == NULL)
    *to = xstrdup (op_arg);
  else
...
static void
gldelf32lm32_before_allocation (void)
{
  const char *rpath;
  asection *sinterp;
  bfd *abfd;
  ...
  lang_for_each_statement (gldelf32lm32_find_statement_assignment);
	...
	if (audit_libs && *audit_libs != '\0')
	  {
		...
		if (cp != NULL && *cp != '\0')
		  gldelf32lm32_append_to_separated_string (&depaudit, cp);
...
static bfd_boolean
gldelf32lm32_open_dynamic_archive
...
static lang_output_section_statement_type *
gldelf32lm32_place_orphan (asection *s,
				   const char *secname,
				   int constraint)
{
...
static void
gldelf32lm32_after_allocation (void)
{
  bfd_boolean need_layout = bfd_elf_discard_info (link_info.output_bfd,
						  &link_info);
  gldelf32lm32_map_segments (need_layout);
}

static char *
gldelf32lm32_get_script (int *isfile)
{
  *isfile = 0;
...
; }
#define gldelf32lm32_add_options NULL
#define gldelf32lm32_handle_option NULL
#define gldelf32lm32_list_options NULL

struct ld_emulation_xfer_struct ld_elf32lm32_emulation =
{
  gldelf32lm32_before_parse,
  syslib_default,
  hll_default,
  after_parse_default,
  gldelf32lm32_after_open,
  gldelf32lm32_after_allocation,
  set_output_arch_default,
  ldemul_default_target,
  gldelf32lm32_before_allocation,
  gldelf32lm32_get_script,
  "elf32lm32",
  "elf32-lm32",
  finish_default,
  NULL,
  gldelf32lm32_open_dynamic_archive,
  gldelf32lm32_place_orphan,
  NULL,
  NULL,
  gldelf32lm32_add_options,
  gldelf32lm32_handle_option,
  NULL,
  gldelf32lm32_list_options,
  gldelf32lm32_load_symbols,
  NULL,
  NULL
};
