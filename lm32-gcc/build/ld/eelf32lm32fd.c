...
/* 32 bit ELF emulation code for elf32lm32fd
...
static void gldelf32lm32fd_before_parse (void);
static void gldelf32lm32fd_after_open (void);
static void gldelf32lm32fd_before_allocation (void);
static void gldelf32lm32fd_after_allocation (void);
static lang_output_section_statement_type *gldelf32lm32fd_place_orphan
  (asection *, const char *, int);

static void
gldelf32lm32fd_map_segments (bfd_boolean need_layout)
{
  int tries = 10;
...
static void
gldelf32lm32fd_before_parse (void)
{
  ldfile_set_output_arch ("lm32", bfd_arch_lm32);
...
static bfd_boolean
gldelf32lm32fd_load_symbols (lang_input_statement_type *entry)
{
  int link_class = 0;
...
static void
gldelf32lm32fd_vercheck (lang_input_statement_type *s)
{
  const char *soname;
...
static void
gldelf32lm32fd_stat_needed (lang_input_statement_type *s)
{
  struct stat st;
  const char *suffix;
  const char *soname;
...
static bfd_boolean
gldelf32lm32fd_try_needed (struct dt_needed *needed,
				 int force)
{
  bfd *abfd;
  ...
  if (! force)
      ...
      if (needs != NULL)
	{
	  ...
	  lang_for_each_input_file (gldelf32lm32fd_vercheck);
	  if (global_vercheck_failed)
	    {
	      bfd_close (abfd);
  ...
  global_found = NULL;
  lang_for_each_input_file (gldelf32lm32fd_stat_needed);
  if (global_found != NULL)
    {
...
static bfd_boolean
gldelf32lm32fd_search_needed (const char *path,
				    struct dt_needed *n, int force)
{
  ...
  if (name[0] == '/')
    return gldelf32lm32fd_try_needed (n, force);
  ...
  while (1)
      ...
      else
	{
      ...
      if (gldelf32lm32fd_try_needed (&needed, force))
	return TRUE;
...
static void
gldelf32lm32fd_check_needed (lang_input_statement_type *s)
{
  const char *soname;
...
gldelf32lm32fd_id_note_section_size (bfd *abfd,
...
static bfd_boolean
gldelf32lm32fd_write_build_id_section (bfd *abfd)
{
...
static void
gldelf32lm32fd_after_open (void)
{
  ...
  if (link_info.emit_note_gnu_build_id)
	...
	{
	  size = gldelf32lm32fd_id_note_section_size (abfd, &link_info);
	  if (size == 0)
	    {
	      ...
	      if (s != NULL && bfd_set_section_alignment (abfd, s, 2))
		{
		  ...
		  t->after_write_object_contents
		    = &gldelf32lm32fd_write_build_id_section;
  ...
  for (l = needed; l != NULL; l = l->next)
	...
	continue;
      ...
      global_needed = l;
      global_found = NULL;
      lang_for_each_input_file (gldelf32lm32fd_check_needed);
      if (global_found != NULL
	  && (bfd_elf_get_dyn_lib_class (global_found->the_bfd)
	      & DYN_AS_NEEDED) == 0)
	continue;
      ...
      if (global_found != NULL)
	{
	  ...
	  if (gldelf32lm32fd_try_needed (&nn, TRUE))
	    continue;
	}
	 ...
	 their use.  See gldelf32lm32fd_vercheck comment.  */
      for (force = 0; force < 2; force++)
	{
	  size_t len;
	  search_dirs_type *search;

	  if (gldelf32lm32fd_search_needed (command_line.rpath_link,
						  &n, force))
	    break;
	  len = strlen (l->name);
	  for (search = search_head; search != NULL; search = search->next)
		...
		continue;
	      ...
	      nn.name = filename;
	      if (gldelf32lm32fd_try_needed (&nn, force))
		break;
	      free (filename);
	    }
	  if (search != NULL)
...
static void
gldelf32lm32fd_find_exp_assignment (etree_type *exp)
{
  ...
  switch (exp->type.node_class)
      ...
      if (strcmp (exp->assign.dst, ".") != 0)
	{
      ...
      gldelf32lm32fd_find_exp_assignment (exp->assign.src);
      break;

    case etree_binary:
      gldelf32lm32fd_find_exp_assignment (exp->binary.lhs);
      gldelf32lm32fd_find_exp_assignment (exp->binary.rhs);
      break;

    case etree_trinary:
      gldelf32lm32fd_find_exp_assignment (exp->trinary.cond);
      gldelf32lm32fd_find_exp_assignment (exp->trinary.lhs);
      gldelf32lm32fd_find_exp_assignment (exp->trinary.rhs);
      break;

    case etree_unary:
      gldelf32lm32fd_find_exp_assignment (exp->unary.child);
      break;
...
static void
gldelf32lm32fd_find_statement_assignment (lang_statement_union_type *s)
{
  if (s->header.type == lang_assignment_statement_enum)
    gldelf32lm32fd_find_exp_assignment (s->assignment_statement.exp);
}


/* used by before_allocation and handle_option. */
static void 
gldelf32lm32fd_append_to_separated_string (char **to, char *op_arg)
{
  if (*to == NULL)
    *to = xstrdup (op_arg);
  else
...
static void
gldelf32lm32fd_before_allocation (void)
{
  const char *rpath;
  asection *sinterp;
  bfd *abfd;
  ...
  lang_for_each_statement (gldelf32lm32fd_find_statement_assignment);
	...
	if (audit_libs && *audit_libs != '\0')
	  {
		...
		if (cp != NULL && *cp != '\0')
		  gldelf32lm32fd_append_to_separated_string (&depaudit, cp);
...
static bfd_boolean
gldelf32lm32fd_open_dynamic_archive
...
static lang_output_section_statement_type *
gldelf32lm32fd_place_orphan (asection *s,
				   const char *secname,
				   int constraint)
{
...
static void
gldelf32lm32fd_after_allocation (void)
{
  bfd_boolean need_layout = bfd_elf_discard_info (link_info.output_bfd,
						  &link_info);
  gldelf32lm32fd_map_segments (need_layout);
}

static char *
gldelf32lm32fd_get_script (int *isfile)
{
  *isfile = 1;

  if (link_info.relocatable && config.build_constructors)
    return "ldscripts/elf32lm32fd.xu";
  else if (link_info.relocatable)
    return "ldscripts/elf32lm32fd.xr";
  else if (!config.text_read_only)
    return "ldscripts/elf32lm32fd.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/elf32lm32fd.xn";
  else if (link_info.pie && link_info.combreloc
	   && link_info.relro && (link_info.flags & DF_BIND_NOW))
    return "ldscripts/elf32lm32fd.xdw";
  else if (link_info.pie && link_info.combreloc)
    return "ldscripts/elf32lm32fd.xdc";
  else if (link_info.pie)
    return "ldscripts/elf32lm32fd.xd";
  else if (link_info.shared && link_info.combreloc
	   && link_info.relro && (link_info.flags & DF_BIND_NOW))
    return "ldscripts/elf32lm32fd.xsw";
  else if (link_info.shared && link_info.combreloc)
    return "ldscripts/elf32lm32fd.xsc";
  else if (link_info.shared)
    return "ldscripts/elf32lm32fd.xs";
  else if (link_info.combreloc && link_info.relro
	   && (link_info.flags & DF_BIND_NOW))
    return "ldscripts/elf32lm32fd.xw";
  else if (link_info.combreloc)
    return "ldscripts/elf32lm32fd.xc";
  else
    return "ldscripts/elf32lm32fd.x";
}
...
static bfd_boolean
gldelf32lm32fd_handle_option (int optc)
{
  switch (optc)
    {
    default:
	...
	link_info.emit_note_gnu_build_id = xstrdup (optarg);
      break;
    case OPTION_AUDIT:
  	gldelf32lm32fd_append_to_separated_string (&audit, optarg); 
	break;
    case 'P':
	gldelf32lm32fd_append_to_separated_string (&depaudit, optarg);
	break;
...
static void
gldelf32lm32fd_list_options (FILE * file)
{
...
struct ld_emulation_xfer_struct ld_elf32lm32fd_emulation =
{
  gldelf32lm32fd_before_parse,
  syslib_default,
  hll_default,
  after_parse_default,
  gldelf32lm32fd_after_open,
  gldelf32lm32fd_after_allocation,
  set_output_arch_default,
  ldemul_default_target,
  gldelf32lm32fd_before_allocation,
  gldelf32lm32fd_get_script,
  "elf32lm32fd",
  "elf32-lm32fdpic",
  finish_default,
  NULL,
  gldelf32lm32fd_open_dynamic_archive,
  gldelf32lm32fd_place_orphan,
  NULL,
  NULL,
  gldelf32lm32fd_add_options,
  gldelf32lm32fd_handle_option,
  NULL,
  gldelf32lm32fd_list_options,
  gldelf32lm32fd_load_symbols,
  NULL,
  NULL
};
